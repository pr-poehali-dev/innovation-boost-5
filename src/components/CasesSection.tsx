...
const cases = [
  {
    ...
    result: 'Интегрировала SCADA-систему с 1С ERP: данные со станков передаются автоматически в реальном времени.',
    metric: '+25%',
  ...
  },
  {
    ...
    result: 'Построила единую BI-платформу: все 4 предприятия в одном дашборде, отчёт генерируется в один клик.',
    metric: '4',
  ...
  },
  {
    ...
    result: 'Провела аудит, пересмотрела вендоров, внедрила Jira + Confluence + SLA. Качество выросло, затраты упали.',
    metric: '−30%',
  ...
  return (
    <section className="bg-slate-50 py-24">
      <div className="container mx-auto px-8 md:px-16">
        ...
        <div className="flex flex-col overflow-hidden rounded-2xl border border-slate-200 shadow-sm">
          {cases.map((item, index) => (
            <div
              key={item.id}
              className="group grid grid-cols-1 border-b border-slate-100 last:border-b-0 md:grid-cols-2"
            >
              <div className={cn('relative h-64 overflow-hidden md:h-80', index % 2 === 1 ? 'md:order-2' : 'md:order-1')}>
                <img
                  src={item.image}
                  alt={item.title}
                ...
                <div className="absolute bottom-6 left-6">
                  <span className="text-5xl font-bold text-white drop-shadow-lg md:text-6xl">{item.metric}</span>
                  <p className="mt-1 text-xs text-blue-100">{item.metricLabel}</p>
                </div>
              </div>

              <div className={cn('flex flex-col justify-center gap-5 bg-white px-8 py-10 md:px-12', index % 2 === 1 ? 'md:order-1' : 'md:order-2')}>
                <span className="text-xs font-semibold uppercase tracking-widest text-blue-600">{item.tag}</span>
                ...
                <div className="space-y-3">
                  <div className="flex gap-3">
                    ...
                    <p className="text-sm leading-relaxed text-slate-500">
                      <span className="font-medium text-slate-600">Проблема: </span>{item.problem}
                    </p>
                  </div>
                  <div className="flex gap-3">
                    <span className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-green-500" />
                    <p className="text-sm leading-relaxed text-slate-600">
                      <span className="font-medium text-slate-700">Решение: </span>{item.result}
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div className="mt-12 flex flex-col items-start gap-4 md:flex-row md:items-center md:justify-between">
          <p className="text-base text-slate-500">Больше шаблонов, чек-листов и разборов — в Telegram-канале</p>
          <a
            href="https://t.me/autopilot_for_business"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 rounded-md border border-blue-600 px-6 py-3 text-sm font-semibold text-blue-600 transition-all hover:bg-blue-600 hover:text-white"
          >

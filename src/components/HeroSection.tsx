...
const stats = [
  ...
  { value: '4', label: 'предприятия в единой платформе' },
];
...
export default function HeroSection() {
  ...
  return (
    <section className="relative h-screen w-full overflow-hidden bg-slate-900">
      ...
      {/* Overlay — светло-синий градиент */}
      <div className="absolute inset-0 bg-gradient-to-r from-blue-900/90 via-blue-800/70 to-blue-700/30" />
          ...
          <div className="flex max-w-2xl flex-col gap-10">
            ...
            <div className={cn('transform transition-all duration-1000 delay-300 ease-out', isLoaded ? 'translate-x-0 opacity-100' : '-translate-x-8 opacity-0')}>
              <div className="space-y-5">
                ...
                <p className="text-3xl font-semibold text-white md:text-4xl lg:text-5xl">
                  Иванова Наталья
                </p>
                ...
                {/* Stats */}
                <div className={cn('flex flex-wrap gap-8 pt-2 transform transition-all duration-1000 delay-500 ease-out', isLoaded ? 'translate-x-0 opacity-100' : '-translate-x-8 opacity-0')}>
                  {stats.map((stat) => (
                ...
                <div className="flex flex-wrap items-center gap-4 pt-4">
                  <a
                    ...
                    className="inline-flex items-center gap-2 rounded-md bg-white px-6 py-3 text-sm font-semibold text-blue-700 shadow-lg transition-all hover:bg-blue-50"
                  >
      ...
      {/* Tags */}
      <div className={cn('absolute bottom-16 left-8 z-20 flex flex-wrap gap-2 md:left-16 transform transition-all duration-1000 delay-700 ease-out', isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0')}>
        {['1С ERP', 'BI-аналитика', 'AI в управлении', 'SCADA + 1С', 'Цифровая трансформация'].map((tag) => (
          <span key={tag} className="rounded-full border border-white/30 bg-white/10 px-3 py-1 text-xs text-white/80 backdrop-blur-sm">
            {tag}
          </span>
        ))}
      </div>
      ...
      <div className="absolute bottom-8 right-8 z-20 flex gap-2">
        {images.map((_, index) => (
          <button
            ...
            className={cn('h-1.5 rounded-full transition-all duration-300', currentIndex === index ? 'w-10 bg-white' : 'w-6 bg-white/40 hover:bg-white/60')}
